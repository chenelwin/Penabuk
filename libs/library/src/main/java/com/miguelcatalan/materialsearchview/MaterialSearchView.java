...
public class MaterialSearchView extends FrameLayout implements Filter.FilterListener {
     ...
     * @param suggestions array of suggestions
     */
    public void setSuggestions(String[] suggestions, boolean checkIfSuggestionContainsQuery){
        if (suggestions != null && suggestions.length > 0) {
            mTintView.setVisibility(VISIBLE);
            final SearchAdapter adapter = new SearchAdapter(mContext, suggestions, suggestionIcon, ellipsize, checkIfSuggestionContainsQuery);
            setAdapter(adapter);
